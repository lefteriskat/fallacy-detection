...
def run_multiple_round(cfg: DictConfig) -> None:
    ...
    output_file = f"final_reports_mr/{str(cfg.model.name).replace('/', '-')}_prompt{prompt_option}_{'basic_cot' if cot else 'no_cot'}_{fallacy_class.name}_results{'_with_definitions' if cfg.prompt.definitions else '_no_definitions'}_{cfg.seed}_temp{temperature}.csv"
