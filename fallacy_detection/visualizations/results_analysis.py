...
def extract_metrics(model_name: str, coarse_grained: bool, definitions: bool, from_fine_to_coarse:bool = False):
    dataframe = pd.read_csv(f"reports/{model_name.replace('/', '-')}_{'coarse_grained' if coarse_grained else 'fine_grained'}_results{'_with_definitions' if definitions else '_no_definitions'}.csv")
    ...
    metrics_dict["accuracy"] = accuracy_score(dataframe["true_label"].to_numpy(), dataframe["predicted_label"].to_numpy())*100
    metrics_dict["f1 score"] = f1_score(dataframe["true_label"].to_numpy(), dataframe["predicted_label"].to_numpy(), average = 'macro')
    ...
    if coarse_grained or from_fine_to_coarse:
        different_labels_count = dataframe["predicted_label"].str.lower().apply(lambda x: x not in ALL_COPI_COARSE_GRAINED_LOGIC_FALLACIES_LOWER and x != "unknown").sum()
    else:
        different_labels_count = dataframe["predicted_label"].str.lower().apply(lambda x: x not in ALL_LOGIC_FALLACIES_LOWER and x != "unknown").sum()
...
def create_plots(model_name: str, coarse_grained: bool, definitions: bool, from_fine_to_coarse:bool = False):
    ...
    if from_fine_to_coarse:
        ...
        title1 = f"Coarse Grained Classes {'with defintions' if definitions else 'without defintions'} True labels Histogram" if coarse_grained else f"Fine Grained Classes {'with defintions' if definitions else 'without defintions'} True labels Histogram"
        title2 = f"Coarse Grained Classes {'with defintions' if definitions else 'without defintions'} Predicted labels Histogram" if coarse_grained else f"Fine Grained Classes {'with defintions' if definitions else 'without defintions'} Predicted labels Histogram"
...
def run_metrics(coarse_grained:bool = False, from_fine_to_coarse: bool = False):
    ...
    for model in TARGET_MODELS:
        results_data.append(extract_metrics(model, coarse_grained, definitions=False, from_fine_to_coarse=from_fine_to_coarse))
        results_data.append(extract_metrics(model, coarse_grained, definitions=True, from_fine_to_coarse=from_fine_to_coarse))
